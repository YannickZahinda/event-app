    ...
    <section className="px-10 py-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {shopData.map((item, index) => {
          ...
          const handleWhatsAppClick = ({title, price, originalPrice, discount}: ProductProps) => {
            const phoneNumber = "+250783020147";
          ...
          return (
            <div key={index} className="bg-gray-300 p-4 rounded-lg shadow-lg text-center relative">
              ...
              <div className="flex justify-between items-center mt-4">
                <div className="flex space-x-1">
                  {[...Array(4)].map((_, i) => (
                    <FaHeart key={i} className="text-gray-500 hover:cursor-pointer hover:text-[#4A1316]" />
                  ))}
                </div>
                <Button text="+"  className="bg-[#4A1316] text-white rounded-full w-10 h-10 flex items-center justify-center text-xl"
